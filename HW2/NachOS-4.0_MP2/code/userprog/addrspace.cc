...
bool AddrSpace::Load(char *fileName)
{
    ...
    //            divRoundUp(noffH.readonlyData.size, PageSize) +
    //            divRoundUp(noffH.initData.size, PageSize) +
    //            divRoundUp(noffH.uninitData.size, PageSize);

// I don't care about readonly data anymore

    //cout << divRoundUp(noffH.code.size, PageSize) <<
    //           divRoundUp(noffH.readonlyData.size, PageSize) <<
    //           divRoundUp(noffH.initData.size, PageSize) <<
    //          divRoundUp(noffH.uninitData.size, PageSize);

    int readonlyStart = divRoundUp(noffH.code.size, PageSize);
    int readonlyEnd   = readonlyStart + divRoundUp(noffH.readonlyData.size, PageSize);
    ...
    if (numPages > kernel->NumFreePages)
    {
...
#ifdef RDATA
        if (i >= readonlyStart && i <= readonlyEnd){
