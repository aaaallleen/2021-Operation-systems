...
AddrSpace::~AddrSpace()
{
    for(int i = 0; i<numPages; i++){
        kernel->freePageNum++;
        kernel->PhyPageUsed[pageTable[i].physicalPage] = FALSE; 
    }
...
// how big is address space?
    ...
    if(numPages > kernel->freePageNum) {
    ...
    for (int i = 0, idx = 0; i < numPages; i++) {
        ...
        while(kernel->PhyPageUsed[idx] == TRUE && idx < NumPhysPages) {
            idx++;
        }
        bzero(&kernel->machine->mainMemory[idx * PageSize], PageSize);
        kernel->PhyPageUsed[idx] = TRUE;
        kernel->freePageNum--;
