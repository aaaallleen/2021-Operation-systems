...
void Thread::Yield()
{
    ...
    int newBurst = kernel->stats->totalTicks - this->s_tick;
    this->setburst( newBurst );
    if(this->getpriority() > 99){
        this->CPUguesssedburst -= newBurst;
        kernel->scheduler->PutBack(this);
    }
    nextThread = kernel->scheduler->FindNextToRun();
    if (nextThread != NULL)
    {   
        // cout<<"\nReplacing thread[" << this->getID() <<"] guess it has to run for " << this->CPUguesssedburst << " more ticks.\n" ;
        // cout<<"The chosen thread is thread[" << nextThread->getID() <<"] guess it has to run for " << nextThread->CPUguesssedburst <<" more ticks. \n";
        if(nextThread!=this){
            kernel->scheduler->ReadyToRun(this);
            kernel->scheduler->Run(nextThread, FALSE);
        }
        // DEBUG(dbgMP3, "[E] Tick [" << kernel->stats->totalTicks << "]: Thread [" << nextThread->getID() << "] is now selected for execution, thread [" << this->getID() << "] is replaced, and it has executed[" << (kernel->stats->userTicks - this->s_tick) << "] ticks");
    }
...
void Thread::Sleep(bool finishing)
{
    ...
    // cout << endl<<"Sleeping Thread["<< name <<"]" << endl;
    ...
    int newBurst = kernel->stats->totalTicks - this->s_tick;
    this->setburst( newBurst );
    double guess = 0.5*burst + 0.5*this->CPUguesssedburst;
    this->CPUguesssedburst = guess;
    ...
    // double remainingBurstTime = burst - (kernel->stats->totalTicks - this->s_tick);
...
void Thread::Aging()
{
    ...
    if (age >= 15)
    {
        ...
        if (priority >= 149)
            priority = 149;
        DEBUG(dbgMP3, "[C] Tick [" << kernel->stats->totalTicks << "]: Thread [" << ID << "] changes its priority from [" << pre << "] to [" << priority << "]");
    }
}
...
void Thread::SelfTest()
{
    ...
    Thread *t = new Thread("forked thread", 1, PRIORITY_MAX);
